...
class SectionRepository{
  ...
  Future<List<Section>> loadSections() async{
    final snapshot = await firestore.collection(FirestorePaths.PATH_SECTIONS).getDocuments();
    final sections = snapshot.documents.map((d)=>sectionFromDoc(d)).toList();
    final returnList = List<Section>();
    for(var section in sections){
      final userSections = await loadUserSectionsBySection(section.id);
      returnList.add(section.rebuild((s)=>s..users = ListBuilder<UserSection>(userSections)));
    }
    return returnList;
  }

  Future<List<UserSection>> loadUserSectionsByUser(String uid) async {
    final snapshot = await firestore.collection(FirestorePaths.PATH_USERSSECTIONS).where(USERSECTION_USERID, isEqualTo: uid).getDocuments();
    return snapshot.documents.map((us)=>_userSectionFromDoc(us)).toList();
  }

  Future<List<UserSection>> loadUserSectionsBySection(String sectionId) async {
    final snapshot = await firestore.collection(FirestorePaths.PATH_USERSSECTIONS).where(USERSECTION_SECTIONID, isEqualTo: sectionId).getDocuments();
    return snapshot.documents.map((us)=>_userSectionFromDoc(us)).toList();
  }

  Future<void> addUserSection(UserSection userSection) async {
    await firestore.collection(FirestorePaths.PATH_USERSSECTIONS).add(_userSectionToMap(userSection));
  }

  Future<void> removeUserSection(String userSectionId) async{
    await firestore.document(FirestorePaths.userSectionPath(userSectionId)).delete();
  }

  Future<void> editSection(Section section) async{
    await firestore.document(FirestorePaths.sectionPath(section.id)).setData(sectionToMap(section));
  }

  Future<Section>addSection(Section section) async{
    final documentReference = await firestore.collection(FirestorePaths.PATH_SECTIONS).add(sectionToMap(section));
