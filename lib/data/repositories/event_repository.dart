...
class EventRepository {
  ...
  Future<List<Event>> loadEvents() async {
    final snapshot = await firestore
        ...
        .get();
    final events = snapshot.docs.map((d) => _eventFromDoc(d)).toList();
    final eventsToReturn = <Event>[];
    for (var event in events) {
      final attendance = await getEventAttendance(event.id);
      final sections = await getEventSections(event.id);
      eventsToReturn.add(event.rebuild((e) => e
        ..attendance = ListBuilder<Attendance>(attendance)
        ..sections = ListBuilder<Section>(sections)));
    }
  ...
  Future<List<Attendance>> getEventAttendance(String eventId) async {
    ...
    return eventAttendance.docs.map((d) => _attendanceFromDoc(d)).toList();
  }
  ...
  Future<void> editEvent(Event event) async {
    await firestore
        .doc(FirestorePaths.eventPath(event.id))
        .update(toMap(event));
  }
  ...
  Future<void> saveAttendance(String eventId, Attendance attendance) async {
    if (attendance.id == null || attendance.id == "") {
      ...
      await firestore
          .doc(FirestorePaths.attendancePath(eventId, attendance.id))
          .update(_attendanceToMap(attendance));
    }
  }
  ...
  Future<Attendance> getUserAttendance(String eventId, String uid) async {
    ...
    final attendanceList =
        eventAttendance.docs.map((d) => _attendanceFromDoc(d)).toList();
    if (attendanceList != null &&
        attendanceList.length > 0 &&
        attendanceList.any((test) => test.uid.contains(uid))) {
  ...
  Future<void> removeEvent(String eventId) async {
    await firestore.doc(FirestorePaths.eventPath(eventId)).delete();
  }
